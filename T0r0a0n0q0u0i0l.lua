...
PremiumPS:CreateToggle("test", {Toggled=false , Description = false}, function(val)
    ...
    while isHeOk do
        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Backpack:FindFirstChild("SprayPaint") then
            for _, v in ipairs(game.Players:GetPlayers()) do
                if v ~= game.Players.LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                    local posofpl = v.Character.HumanoidRootPart.CFrame
                    for _, normalId in ipairs({Enum.NormalId.Top, Enum.NormalId.Bottom, Enum.NormalId.Left, Enum.NormalId.Right, Enum.NormalId.Front}) do
                        for _, offset in ipairs({-0.7, -1, -1.5, -2, -2.5}) do
                            game:GetService("ReplicatedStorage").Remotes.Extras.ReplicateToy:InvokeServer("SprayPaint")
                            game.Players.LocalPlayer.Backpack.SprayPaint.Parent = game.Players.LocalPlayer.Character
                            game.Players.LocalPlayer.Character.SprayPaint.Remote:FireServer(12976059241, normalId, 0.5, v.Character.HumanoidRootPart, posofpl * CFrame.new(0, -1, offset))
                            wait(0.1) -- You might want to adjust this wait time based on the game's latency
                        end
                    end
                end
            end
