...
local Toggle = Page:AddToggle("Toggle Fake Gun", false, function(val)
if val then
...
local Button = Page:AddButton("Get Fake Gun", function()
game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Gameplay"):WaitForChild("FakeGun"):FireServer(unpack({
    [1] = true -- set to true to activate
}))
end)

local Button = Page:AddButton("Drop Fake Gun", function()
game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Gameplay"):WaitForChild("FakeGun"):FireServer(unpack({
    [1] = true -- set to true to activate
}))
...
local Toggle = Page:AddToggle("Fake Gun Rain", false, function(Value)
...
local Button = Page:AddButton("Clear Dropped Guns (Server Side)", function()
...
local Button = Page:AddButton("Clear Dropped Guns (Client Side)", function()
...
local Button = Page:AddButton("Clear Fake Guns From Inventory", function()
local player = game.Players.LocalPlayer
...
local Toggle = Page:AddToggle("Loop Traps", false, function(Value)
    trapToggle = Value
    if trapToggle then
...
local TextBox = Page:AddTextBox("Loop Trap Delay (0-10)", function(Value)
...
-- Add a TextBox for inputting the player's name
local TextBox = Page:AddTextBox("Username (All)", function(Value)
    fetargetname = Value
    print("Player name set to:", fetargetname)
end)
...
local Button = Page:AddButton("Reset", function()
...
local Toggle = Page:AddToggle("Loop Reset", false, function(resetplayer)
...
local Button = Page:AddButton("Remove Collisions (15sec)", function()
    if fetargetname == "All" then
        EquipSpray() -- Equip the spray first
...
local Toggle = Page:AddToggle("Loop Remove Collisions", false, function(Value)
    if Value == true then
...
local Toggle = Page:AddToggle("Blind", false, function(Value)
    if Value == true then
        poolplayerloop = true
...
local Toggle = Page:AddToggle("Slip Up", false, function(Value)
    if Value == true then
        fesliploop = true
        while fesliploop do
            EquipSpray()
...
local Toggle = Page:AddToggle("Void", false, function(Value)
    if Value == true then
...
local TextBox = Page:AddTextBox("Username (All)", function(Value)
    ...
    print("Player name set to:", infinityGauntlet)
end)
